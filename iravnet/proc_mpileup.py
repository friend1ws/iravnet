...
def proc_mpileup(input_file, output_file, seqlen, min_variant_num = 3, min_variant_ratio = 0.05):
    ...
    with open(input_file, 'r') as hin:
        for line in hin:
            ...
            if mis_rate < min_variant_ratio: continue
            
            if len([x for x in base2pos[alt] + base2pos[alt.lower()] if int(x) >= 10 and int(x) <= seqlen - 10]) == 0: continue
            ...
            if float(var_p + var_n) / int(F[3]) < min_variant_ratio: continue

            print('\t'.join([F[0], F[1], F[1], F[2], alt, str(round(mis_rate, 4)), str(depth_p + depth_n), 
                             str(var_p + var_n), str(round(strand_ratio, 4)), var_info]), file = hout)
